    ...
    internal class Hud_Mission
    {
        ...
        public unsafe static void Draw()
        {
            if (GenericHelpers.TryGetAddonMaster<WKSMission>("WKSMission", out var x) && x.IsAddonReady)
            {
                ...
                if (ImGui.Checkbox("启用虚拟XP", ref EnableDummyXp))
                {
                ...
                if (ImGui.Checkbox("仅启用任务", ref onlyEnabled))
                {
                    C.XPRelicOnlyEnabled = onlyEnabled;
                    C.Save();
                }

                if (ImGui.Button("更新虚拟人物XP"))
                {
                ...
                foreach (var key in C.DummyXP.Keys.ToList())
                {
                    ...
                    if (ImGui.InputInt("当前XP", ref currentXP))
                    {
                    ...
                    ImGui.SetNextItemWidth(100);
                    if (ImGui.InputInt("需要XP", ref neededXP))
                    {
                ...
                foreach (var m in x.StellerMissions)
                {
                    ...
                    ImGui.SameLine();
                    if (ImGui.Button($"发起##Initiate + {m.Name}"))
                    {
                        m.Initiate();
                    }
                }
